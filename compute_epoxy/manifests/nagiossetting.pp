class compute_epoxy::nagiossetting inherits compute_epoxy::params {

     file {'nagios_check_ovs':
             source      => 'puppet:///modules/compute_epoxy/nagios_check_ovs.sh',
             path => '/usr/local/bin/nagios_check_ovs.sh',
             mode => '+x',
             #subscribe   => Class['compute_epoxy::neutron'],
          }

     file {'nagios_check_kvm':
             source      => 'puppet:///modules/compute_epoxy/check_kvm',
             path        => '/usr/local/bin/check_kvm',
             mode        => '+x',
             #subscribe   => Class['compute_epoxy::nova'],
          }

     file {'nagios_check_kvm_wrapper':
             source      => 'puppet:///modules/compute_epoxy/check_kvm_wrapper.sh',
     ...
     file {'check_total_disksize.sh':
             source      => 'puppet:///modules/compute_epoxy/check_total_disksize.sh',
...
### FEDE vedi ticket PDCL-2010
     if $operatingsystemrelease =~ /^9.*/ {
         file {'check_lvs.py':
                 content  => template('compute_epoxy/check_lvs.py.template'),
                 #source      => 'puppet:///template/compute_epoxy/check_lvs.py.template',
